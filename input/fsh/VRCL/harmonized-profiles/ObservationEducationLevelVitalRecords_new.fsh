...
* code from ValueSetEducationLevelPersonVitalRecords (required) //updated this valueset to include code for decedent's highest level of education, 80913-7
  * ^short = "Persons for whom education level can be reported."
  * ^binding.description = "EducationLevelPerson"
* subject 1..1
* subject only Reference(PatientChildVitalRecords or PatientDecedentFetusVitalRecords or Decedent) 
  * ^short = "This is the subject of the report, not the parent"
* focus 1..1
* focus only Reference(RelatedPersonMotherVitalRecords or RelatedPersonFatherVitalRecords) //not required, may be omitted if reporting education level of decedent, not parent. 
